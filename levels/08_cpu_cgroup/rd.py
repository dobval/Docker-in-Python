...
def contain(command, image_name, image_dir, container_id, container_dir,
            cpu_shares):
    # Insert the container to a new cpu cgroup named:
    #       'docker-python/container_id'
    # Create the cgroup directory structure
    cgroup_path = f"/sys/fs/cgroup/cpu/docker-python/{container_id}"
    os.makedirs(cgroup_path, exist_ok=True)

    with open(os.path.join(cgroup_path, 'tasks'), 'w') as tasks_file:
        tasks_file.write(str(os.getpid()))

    # if (cpu_shares != 0)  => set the 'cpu.shares' in our cpu cgroup
    if (cpu_shares != 0):
        with open(os.path.join(cgroup_path, 'cpu.shares'), 'w') as shares_file:
            shares_file.write(str(cpu_shares))
