...
def _setup_cpu_cgroup(container_id, cpu_shares):
    ...
    container_cpu_cgroup_dir = os.path.join(
        CPU_CGROUP_BASEDIR, 'docker-python', container_id)

    # Insert the container to new cpu cgroup named 'docker-python/container_id'
...
def contain(command, image_name, image_dir, container_id, container_dir,
    ...
    # Similarly to the CPU cgroup, add Memory cgroup support here
    #       setup memory -> memory.limit_in_bytes,
    #       memory_swap -> memory.memsw.limit_in_bytes if they are not None
    cgroup_path = f"/sys/fs/cgroup/memory/docker-python/{container_id}"
    os.makedirs(cgroup_path, exist_ok=True)

    with open(f"/sys/fs/cgroup/memory/docker-python/{container_id}/tasks", 'w') as tasks_file:
        tasks_file.write(str(os.getpid()))

    if memory is not None:
        with open(f"/sys/fs/cgroup/memory/docker-python/{container_id}/memory.limit_in_bytes", 'w') as limit_file:
            limit_file.write(str(memory))

    if memory_swap is not None:
        with open(f"/sys/fs/cgroup/memory/docker-python/{container_id}/memory.memsw.limit_in_bytes", 'w') as swap_file:
            swap_file.write(str(memory_swap))
